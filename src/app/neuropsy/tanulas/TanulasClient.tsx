  ...
  return (
    <main style={{
      ...
      <div style={{ width: "100%", maxWidth: 960, display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 8, marginBottom: 12, flexWrap: "wrap", gap: 8 }}>
        ...
        <Link href="/neuropsy/vizsga" style={{ padding: "8px 18px", background: "linear-gradient(90deg,#7c3aed,#9333ea)", color: "white", border: "none", borderRadius: 10, fontWeight: 600, textDecoration: "none" }}>
          Vizsga mód →
        </Link>
      </div>
      ...
      <div style={{ background: "white", borderRadius: 20, boxShadow: "0 6px 24px rgba(139,92,246,0.10)", padding: "24px", maxWidth: 960, width: "100%" }}>
        ...
        <div style={{ fontSize: 14, color: "#5b21b6", marginBottom: 16 }}>
          Neuropszichológia
        </div>

        {/* Tabs */}
        <div style={{ display: "flex", gap: 8, marginBottom: 20, borderBottom: "2px solid #ddd6fe" }}>
          {(["kerdesek", "segedlet"] as const).map((t) => (
            <button
              ...
              style={{
                padding: "8px 18px",
                background: tab === t ? "linear-gradient(90deg,#8b5cf6,#a78bfa)" : "transparent",
                color: tab === t ? "white" : "#6d28d9",
                border: "none",
          ...
          <>
            ...
            <div style={{ display: "flex", gap: 10, marginBottom: 14, flexWrap: "wrap" }}>
              <input
                ...
                style={{ flex: "1 1 180px", padding: "8px 12px", borderRadius: 8, border: "1.5px solid #c4b5fd", fontSize: 14, color: "#4c1d95" }}
              />
              <select
            ...
            <ol style={{ paddingLeft: 0, listStyle: "none", margin: 0 }}>
              {paginated.map((q) => (
                <li key={q.id} style={{ background: "#faf5ff", border: "1.5px solid #ddd6fe", borderRadius: 12, padding: "14px 16px", marginBottom: 12 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", gap: 8, alignItems: "flex-start", marginBottom: 6 }}>
                    <div style={{ fontWeight: 700, color: "#4c1d95", fontSize: 15 }}>
                      <span style={{ color: "#7c3aed", marginRight: 6 }}>{q.id}.</span>
                      {q.question}
                    </div>
                  ...
                  <div style={{ fontSize: 12, color: "#5b21b6", marginBottom: q.source ? 2 : 8 }}>{q.topicNum}. {q.topic}</div>
                  {q.source && (
                  ...
                  {(q.type === "single" || q.type === "multi") && (
                    <ul style={{ listStyle: "none", paddingLeft: 0, margin: "6px 0 0 0", display: "flex", flexWrap: "wrap", gap: 6 }}>
                      {q.options.map((opt, i) => {
                        ...
                        return (
                          <li key={i} style={{ padding: "4px 10px", borderRadius: 6, background: isCorrect ? "#ddd6fe" : "#f3f4f6", color: isCorrect ? "#166534" : "#374151", fontWeight: isCorrect ? 700 : 400, fontSize: 13 }}>
                  ...
                  {q.type === "tf" && (
                    <div style={{ display: "flex", gap: 8, marginTop: 6 }}>
                      {[true, false].map((v) => {
                        ...
                        return (
                          <span key={String(v)} style={{ padding: "4px 14px", borderRadius: 6, background: isCorrect ? "#ddd6fe" : "#f3f4f6", color: isCorrect ? "#166534" : "#374151", fontWeight: isCorrect ? 700 : 400, fontSize: 13 }}>
                            {v ? "Igaz" : "Hamis"}
                          </span>
                        );
                      })}
                  ...
                  {q.type === "matching" && q.leftItems && q.rightItems && (
                    <div style={{ marginTop: 8 }}>
                      <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                        <thead>
                          <tr>
                            <th style={{ textAlign: "left", padding: "4px 8px", background: "#ddd6fe", borderRadius: "6px 0 0 0", color: "#4c1d95" }}>Kategória</th>
                            <th style={{ textAlign: "left", padding: "4px 8px", background: "#ddd6fe", borderRadius: "0 6px 0 0", color: "#4c1d95" }}>Helyes párok</th>
                          </tr>
                        </thead>
                        <tbody>
                          {q.leftItems.map((left, li) => {
                            ...
                            return (
                              <tr key={li} style={{ borderBottom: "1px solid #ddd6fe" }}>
                                <td style={{ padding: "5px 8px", fontWeight: 600, color: "#4c1d95", verticalAlign: "top", width: "40%" }}>{left}</td>
                  ...
                  {q.type === "essay" && (
                    <div style={{ marginTop: 6, padding: "8px 12px", background: "#faf5ff", border: "1px dashed #9333ea", borderRadius: 7, color: "#5b21b6", fontSize: 13, fontStyle: "italic" }}>
                  ...
                  {!hideAnswer && q.explanation && (
                    q.type === "essay" ? (
                      <div style={{ marginTop: 8, padding: "12px 16px", background: "#f5f3ff", border: "1px solid #c4b5fd", borderRadius: 8, color: "#581c87", fontSize: 14, lineHeight: 1.65 }}>
                        <div style={{ fontWeight: 700, marginBottom: 8, color: "#6d28d9", fontSize: 13, textTransform: "uppercase", letterSpacing: 0.5 }}>Mintaválasz</div>
                        {q.explanation.split(/\n\n+/).map((para, i) => (
                          <p key={i} style={{ margin: i === 0 ? "0 0 10px 0" : "0 0 10px 0", textAlign: "justify" }}>{para}</p>
                        ))}
                      </div>
                    ) : (
                      <div style={{ marginTop: 8, padding: "6px 10px", background: "#f5f3ff", border: "1px solid #c4b5fd", borderRadius: 7, color: "#581c87", fontSize: 12 }}>
            ...
            {totalPages > 1 && (
              <div style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: 16, marginTop: 20 }}>
                <button onClick={() => setPage(p => p - 1)} disabled={page === 1} style={{ padding: "8px 16px", borderRadius: 8, border: "none", background: page === 1 ? "#e0e7ff" : "#7c3aed", color: page === 1 ? "#a5b4fc" : "#fff", fontWeight: 600, cursor: page === 1 ? "not-allowed" : "pointer" }}>Előző</button>
                <span style={{ fontWeight: 500, color: "#4c1d95", fontSize: 14 }}>Oldal {page} / {totalPages}</span>
                <button onClick={() => setPage(p => p + 1)} disabled={page === totalPages} style={{ padding: "8px 16px", borderRadius: 8, border: "none", background: page === totalPages ? "#e0e7ff" : "#7c3aed", color: page === totalPages ? "#a5b4fc" : "#fff", fontWeight: 600, cursor: page === totalPages ? "not-allowed" : "pointer" }}>Következő</button>
              </div>
            )}
          </>
        )}
...
function SegedletEmbed() {
  return (
    <div style={{ textAlign: "center", padding: "32px 0" }}>
      <div style={{ fontSize: 16, color: "#5b21b6", marginBottom: 20 }}>
