...
export default function SegedletClient({ sections }: { sections: Section[] }) {
  ...
  return (
    <main
      style={{
        minHeight: "100vh",
        background: "linear-gradient(120deg, #ede9fe 0%, #ede9fe 100%)",
        padding: "16px",
        display: "flex",
    ...
    >
      <div style={{ width: "100%", maxWidth: 1100, display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12, flexWrap: "wrap", gap: 8 }}>
        <Link href="/neuropsy/tanulas" style={{ color: "#6d28d9", fontWeight: 600, textDecoration: "none", background: "white", padding: "8px 16px", borderRadius: 10, border: "1.5px solid #f9a8d4" }}>
          ← Tanuló mód
        </Link>
        <input
          type="text"
      ...
      <div style={{ width: "100%", maxWidth: 1100, display: "grid", gridTemplateColumns: "minmax(220px, 280px) 1fr", gap: 16, alignItems: "start" }}>
              ...
              <li key={s.num}>
                <button
                  ...
                  style={{
                    ...
                    padding: "8px 10px",
                    margin: "2px 0",
                    background: active === s.num ? "linear-gradient(90deg,#9333ea,#a78bfa)" : "transparent",
        ...
        {/* Content */}
        <article style={{ background: "white", borderRadius: 16, boxShadow: "0 4px 16px rgba(139,92,246,0.10)", padding: "28px 32px", color: "#1f2937", lineHeight: 1.6 }}>
          {filteredSections ? (
            <>
            ...
            <>
              <h2 style={{ color: "#831843", fontSize: 24, fontWeight: 700, marginBottom: 16, borderBottom: "2px solid #c4b5fd", paddingBottom: 10 }}>
                {current.num}. {current.title}
              </h2>
              {blocks.map((b) => {
                if (b.kind === "sub") {
                  return (
                    <h3 key={b.key} style={{ color: "#6d28d9", fontSize: 17, fontWeight: 700, marginTop: 20, marginBottom: 8 }}>
                      {b.title}
                    </h3>
                  );
                }
              ...
              <div style={{ marginTop: 28, display: "flex", justifyContent: "space-between", gap: 12 }}>
                ...
                <button
                  ...
                  style={{
                    padding: "10px 18px",
                    background: active === sections.length ? "#f1f5f9" : "linear-gradient(90deg,#9333ea,#a78bfa)",
