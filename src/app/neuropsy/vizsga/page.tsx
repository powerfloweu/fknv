...
export default function NeuropsyVizsgaPage() {
  ...
  return (
    <main style={{
      ...
      <div style={{ background: "white", borderRadius: 24, boxShadow: "0 8px 32px rgba(139,92,246,0.12)", padding: "40px 32px 32px 32px", maxWidth: 500, width: "100%", textAlign: "center", marginBottom: 24 }}>
        <div style={{ marginBottom: 16 }}>
          <svg width="90" height="70" viewBox="0 0 90 70" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ display: "block", margin: "0 auto 12px auto" }}>
            ...
            <path d="M45 7 Q50 20 45 35 Q40 50 45 63" stroke="#7c3aed" strokeWidth="2.5" fill="none" />
            <path d="M30 15 Q35 30 30 50" stroke="#7c3aed" strokeWidth="2" fill="none" />
            <path d="M60 15 Q55 30 60 50" stroke="#7c3aed" strokeWidth="2" fill="none" />
          </svg>
        </div>
        <h1 style={{ fontSize: 24, fontWeight: 700, color: "#6d28d9", marginBottom: 6 }}>
          Vizsga indítása
        </h1>
        <div style={{ fontSize: 15, color: "#7c3aed", fontWeight: 500, marginBottom: 22 }}>
          Neuropszichológia
        </div>

        {/* Filters */}
        <div style={{ textAlign: "left", marginBottom: 18 }}>
          <label style={{ display: "block", fontSize: 14, fontWeight: 600, color: "#4c1d95", marginBottom: 4 }}>Témakör szűrő</label>
          <select
            value={topicFilter === "" ? "" : String(topicFilter)}
            onChange={(e) => setTopicFilter(e.target.value === "" ? "" : Number(e.target.value))}
            style={{ width: "100%", padding: "9px 12px", borderRadius: 8, border: "1.5px solid #c4b5fd", fontSize: 14, color: "#4c1d95", background: "white", marginBottom: 12 }}
          >
            <option value="">Minden témakör</option>
            {topics.map((t) => <option key={t.num} value={t.num}>{t.num}. {t.name}</option>)}
          </select>

          <label style={{ display: "block", fontSize: 14, fontWeight: 600, color: "#4c1d95", marginBottom: 4 }}>Nehézség szűrő</label>
          <select
            value={diffFilter}
          ...
          <label style={{ display: "block", fontSize: 14, fontWeight: 600, color: "#4c1d95", marginBottom: 4 }}>Kérdéstípus szűrő</label>
          <select
            value={typeFilter}
          ...
          <label style={{ display: "block", fontSize: 14, fontWeight: 600, color: "#4c1d95", marginBottom: 4 }}>Kategória szűrő</label>
          <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 12 }}>
            <label style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 12px", background: onlyUj ? "#ede9fe" : "#f5f3ff", borderRadius: 8, border: "1.5px solid #c4b5fd", fontSize: 14, color: "#4c1d95", cursor: "pointer", fontWeight: 500 }}>
          ...
          <label style={{ display: "block", fontSize: 14, fontWeight: 600, color: "#4c1d95", marginBottom: 4 }}>
          ...
          <input
            ...
            style={{ width: "100%", padding: "9px 12px", borderRadius: 8, border: "1.5px solid #c4b5fd", fontSize: 16, color: "#4c1d95", textAlign: "center", boxSizing: "border-box" }}
          />
        </div>
        ...
        <Link href="/neuropsy" style={{ display: "inline-block", marginTop: 18, color: "#64748b", fontSize: 14, textDecoration: "none" }}>
          ← Vissza
        </Link>
      </div>
    </main>
