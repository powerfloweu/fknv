...
function MatchingInput({ q, sel, setSel, submitted }: {
  ...
  return (
    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14, marginTop: 8 }}>
      <thead>
        <tr>
          <th style={{ textAlign: "left", padding: "5px 8px", background: "#ddd6fe", color: "#4c1d95", borderRadius: "6px 0 0 0" }}>Kategória</th>
          <th style={{ textAlign: "left", padding: "5px 8px", background: "#ddd6fe", color: "#4c1d95", borderRadius: "0 6px 0 0" }}>Válasz(ok)</th>
        </tr>
      </thead>
      <tbody>
...
function NormalVizsga() {
  ...
  return (
    <main style={{ minHeight: "100vh", background: "linear-gradient(120deg, #ede9fe 0%, #f5f3ff 100%)", display: "flex", flexDirection: "column", alignItems: "center", padding: "16px" }}>
      <div style={{ width: "100%", maxWidth: 860, display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 8, marginBottom: 12 }}>
        <Link href="/neuropsy/vizsga" style={{ color: "#6d28d9", fontWeight: 600, textDecoration: "none", background: "white", padding: "8px 16px", borderRadius: 10, border: "1.5px solid #c4b5fd" }}>
          ← Vizsga beállítások
        </Link>
        {submitted && score && (
      ...
      <div style={{ background: "white", borderRadius: 20, boxShadow: "0 6px 24px rgba(139,92,246,0.10)", padding: "24px", maxWidth: 860, width: "100%" }}>
        <h1 style={{ fontSize: 22, fontWeight: 700, color: "#6d28d9", marginBottom: 4 }}>Normál vizsga</h1>
        <div style={{ fontSize: 14, color: "#5b21b6", marginBottom: 20 }}>{questions.length} kérdés — jelöld be a válaszokat, majd kérj kiértékelést</div>
        ...
        <ol style={{ listStyle: "none", paddingLeft: 0, margin: 0 }}>
          {questions.map((q, qi) => {
            ...
            function optBg(idx1: number) {
              ...
              if (!reveal) return isSel ? "#ddd6fe" : "#f9fafb";
              if (isAns) return "#ddd6fe";
            ...
            function optColor(idx1: number) {
              const isAns = q.type === "single" ? q.answer === idx1 : Array.isArray(q.answer) && (q.answer as number[]).includes(idx1);
              const isSel = q.type === "single" ? sel === idx1 : Array.isArray(sel) && (sel as number[]).includes(idx1);
              if (!reveal) return isSel ? "#4c1d95" : "#374151";
              if (isAns) return "#166534";
              if (isSel && !isAns) return "#991b1b";
              return "#374151";
            }
            function tfBg(v: boolean) {
              if (!reveal) return sel === v ? "#ddd6fe" : "#f9fafb";
              if (q.answer === v) return "#ddd6fe";
              ...
              <li key={q.id} style={{ background: "#faf5ff", border: "1.5px solid #ddd6fe", borderRadius: 12, padding: "14px 16px", marginBottom: 12 }}>
                <div style={{ display: "flex", justifyContent: "space-between", gap: 8, alignItems: "flex-start", marginBottom: 6 }}>
                  <div style={{ fontWeight: 700, color: "#4c1d95", fontSize: 15 }}>
                    <span style={{ color: "#7c3aed", marginRight: 6 }}>{qi + 1}.</span>{q.question}
                  </div>
                ...
                {q.type === "tf" && (
                  <div style={{ display: "flex", gap: 8, marginTop: 6 }}>
                    {([true, false] as const).map((v) => (
                      <label key={String(v)} style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 16px", borderRadius: 8, cursor: submitted ? "default" : "pointer", background: tfBg(v), color: tfBg(v) === "#ddd6fe" ? "#166534" : tfBg(v) === "#fecaca" ? "#991b1b" : "#374151", fontSize: 14 }}>
        ...
        <div style={{ display: "flex", gap: 10, justifyContent: "center", marginTop: 20 }}>
          {!submitted ? (
            <button onClick={() => setSubmitted(true)} style={{ padding: "12px 28px", background: "linear-gradient(90deg,#7c3aed,#9333ea)", color: "white", border: "none", borderRadius: 12, fontWeight: 700, fontSize: 16, cursor: "pointer" }}>
              Kiértékelés
            </button>
          ) : (
            <button onClick={() => router.push("/neuropsy/vizsga")} style={{ padding: "12px 28px", background: "white", color: "#6d28d9", border: "1.5px solid #c4b5fd", borderRadius: 12, fontWeight: 700, fontSize: 16, cursor: "pointer" }}>
              Új vizsga
            </button>
          )}
        </div>
