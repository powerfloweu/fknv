...
function MatchingInput({ q, sel, setSel, revealed }: {
  ...
  return (
    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14, marginBottom: 16, textAlign: "left" }}>
      <thead>
        <tr>
          <th style={{ padding: "5px 8px", background: "#bbf7d0", color: "#064e3b", borderRadius: "6px 0 0 0" }}>Kategória</th>
          <th style={{ padding: "5px 8px", background: "#bbf7d0", color: "#064e3b", borderRadius: "0 6px 0 0" }}>Válasz(ok)</th>
        </tr>
      </thead>
      <tbody>
        {(q.leftItems || []).map((left, li) => {
          ...
          return (
            <tr key={li} style={{ borderBottom: "1px solid #bbf7d0" }}>
              <td style={{ padding: "6px 8px", fontWeight: 600, color: "#064e3b", verticalAlign: "top", width: "35%" }}>{left}</td>
              <td style={{ padding: "6px 8px" }}>
                {(q.rightItems || []).map((right, ri) => {
                  ...
                  } else if (isChosen) { bg = "#bbf7d0"; color = "#064e3b"; }
                  return (
...
function QaVizsga() {
  ...
  if (finished) {
    return (
      <main style={{ minHeight: "100vh", background: "linear-gradient(120deg, #d1fae5 0%, #ecfeff 100%)", display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", padding: "24px" }}>
        <div style={{ background: "white", borderRadius: 24, boxShadow: "0 8px 32px rgba(16,185,129,0.12)", padding: "40px 32px", maxWidth: 500, width: "100%", textAlign: "center" }}>
          <h1 style={{ fontSize: 26, fontWeight: 700, color: "#047857", marginBottom: 10 }}>Q&amp;A vizsga vége!</h1>
          <div style={{ fontSize: 20, color: "#0d9488", fontWeight: 600, marginBottom: 20 }}>
          ...
          <div style={{ display: "flex", gap: 10, justifyContent: "center" }}>
            <button onClick={() => router.push("/kognitiv/vizsga")} style={{ padding: "12px 24px", background: "linear-gradient(90deg,#10b981,#34d399)", color: "white", border: "none", borderRadius: 12, fontWeight: 700, cursor: "pointer", fontSize: 15 }}>
              Új vizsga
            </button>
            <Link href="/kognitiv/tanulas" style={{ padding: "12px 24px", background: "#ecfdf5", color: "#047857", border: "1.5px solid #a7f3d0", borderRadius: 12, fontWeight: 700, fontSize: 15, textDecoration: "none" }}>
              Tanuló mód
            </Link>
          </div>
        </div>
  ...
  function optStyle(idx1: number) {
    ...
    if (!revealed) return { bg: isSel ? "#bbf7d0" : "#f9fafb", color: isSel ? "#064e3b" : "#374151", border: isSel ? "2px solid #0d9488" : "2px solid transparent" };
  ...
  return (
    <main style={{ minHeight: "100vh", background: "linear-gradient(120deg, #d1fae5 0%, #ecfeff 100%)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "16px" }}>
      <div style={{ background: "white", borderRadius: 24, boxShadow: "0 8px 32px rgba(16,185,129,0.12)", padding: "32px 28px", maxWidth: 700, width: "100%", textAlign: "center" }}>
        {/* Progress */}
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          <Link href="/kognitiv/vizsga" style={{ color: "#64748b", fontSize: 13, textDecoration: "none" }}>← Kilépés</Link>
          <div style={{ fontSize: 14, color: "#065f46", fontWeight: 600 }}>{current + 1} / {questions.length}</div>
          <div style={{ fontSize: 13, color: "#64748b" }}>✓ {score} helyes</div>
        </div>

        {/* Progress bar */}
        <div style={{ height: 6, background: "#bbf7d0", borderRadius: 3, marginBottom: 20, overflow: "hidden" }}>
          <div style={{ height: "100%", background: "linear-gradient(90deg,#0d9488,#10b981)", borderRadius: 3, width: `${(current / questions.length) * 100}%`, transition: "width 0.3s" }} />
        </div>
        ...
        <div style={{ fontWeight: 700, color: "#064e3b", fontSize: 18, marginBottom: 20, lineHeight: 1.4 }}>{q.question}</div>
        ...
        {needsConfirm && !revealed && (
          <button onClick={reveal} disabled={!confirmEnabled}
            style={{ marginBottom: 12, padding: "8px 20px", borderRadius: 8, background: "#0d9488", color: "white", fontWeight: 600, border: "none", cursor: confirmEnabled ? "pointer" : "not-allowed", opacity: confirmEnabled ? 1 : 0.5 }}>
            Válasz beküldése
          </button>
        )}
        ...
        {!needsConfirm && !revealed && sel !== null && (
          <button onClick={reveal} style={{ marginBottom: 12, padding: "8px 20px", borderRadius: 8, background: "#0d9488", color: "white", fontWeight: 600, border: "none", cursor: "pointer" }}>
            Ellenőrzés
          </button>
        )}
        ...
        {revealed && (
          <div style={{ marginBottom: 16 }}>
            ...
            <button onClick={next} style={{ marginTop: 14, padding: "10px 28px", background: "linear-gradient(90deg,#10b981,#34d399)", color: "white", border: "none", borderRadius: 12, fontWeight: 700, fontSize: 15, cursor: "pointer" }}>
