...
function MatchingInput({ q, sel, setSel, submitted }: {
  ...
  return (
    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14, marginTop: 8 }}>
      <thead>
        <tr>
          <th style={{ textAlign: "left", padding: "5px 8px", background: "#bbf7d0", color: "#064e3b", borderRadius: "6px 0 0 0" }}>Kategória</th>
          <th style={{ textAlign: "left", padding: "5px 8px", background: "#bbf7d0", color: "#064e3b", borderRadius: "0 6px 0 0" }}>Válasz(ok)</th>
        </tr>
      </thead>
      <tbody>
...
function NormalVizsga() {
  ...
  return (
    <main style={{ minHeight: "100vh", background: "linear-gradient(120deg, #d1fae5 0%, #ecfeff 100%)", display: "flex", flexDirection: "column", alignItems: "center", padding: "16px" }}>
      <div style={{ width: "100%", maxWidth: 860, display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 8, marginBottom: 12 }}>
        <Link href="/kognitiv/vizsga" style={{ color: "#047857", fontWeight: 600, textDecoration: "none", background: "white", padding: "8px 16px", borderRadius: 10, border: "1.5px solid #86efac" }}>
          ← Vizsga beállítások
        </Link>
        {submitted && score && (
      ...
      <div style={{ background: "white", borderRadius: 20, boxShadow: "0 6px 24px rgba(16,185,129,0.10)", padding: "24px", maxWidth: 860, width: "100%" }}>
        <h1 style={{ fontSize: 22, fontWeight: 700, color: "#047857", marginBottom: 4 }}>Normál vizsga</h1>
        <div style={{ fontSize: 14, color: "#065f46", marginBottom: 20 }}>{questions.length} kérdés — jelöld be a válaszokat, majd kérj kiértékelést</div>
        ...
        <ol style={{ listStyle: "none", paddingLeft: 0, margin: 0 }}>
          {questions.map((q, qi) => {
            ...
            function optBg(idx1: number) {
              ...
              if (!reveal) return isSel ? "#bbf7d0" : "#f9fafb";
              ...
              <li key={q.id} style={{ background: "#f0fdfa", border: "1.5px solid #bbf7d0", borderRadius: 12, padding: "14px 16px", marginBottom: 12 }}>
                <div style={{ display: "flex", justifyContent: "space-between", gap: 8, alignItems: "flex-start", marginBottom: 6 }}>
                  <div style={{ fontWeight: 700, color: "#064e3b", fontSize: 15 }}>
                    <span style={{ color: "#0d9488", marginRight: 6 }}>{qi + 1}.</span>{q.question}
                  </div>
        ...
        <div style={{ display: "flex", gap: 10, justifyContent: "center", marginTop: 20 }}>
          {!submitted ? (
            <button onClick={() => setSubmitted(true)} style={{ padding: "12px 28px", background: "linear-gradient(90deg,#0d9488,#14b8a6)", color: "white", border: "none", borderRadius: 12, fontWeight: 700, fontSize: 16, cursor: "pointer" }}>
              Kiértékelés
            </button>
          ) : (
            <button onClick={() => router.push("/kognitiv/vizsga")} style={{ padding: "12px 28px", background: "white", color: "#047857", border: "1.5px solid #86efac", borderRadius: 12, fontWeight: 700, fontSize: 16, cursor: "pointer" }}>
              Új vizsga
            </button>
          )}
        </div>
